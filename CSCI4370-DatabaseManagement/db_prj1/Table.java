...
//        index     = new LinHashMap <> (KeyType.class, Comparable [].class);
    ...
    public Table select (KeyType keyVal)
    {
        ...
        List <Comparable []> rows = new ArrayList <> ();
	Comparable[] matchedTuple = index.get(keyVal);

        if (matchedTuple != null) {
            rows.add(matchedTuple);
        }
    ...
    // Find common attributes and their indexes in both tables
    List<String> commonAttributes = new ArrayList<>();
    List<Integer> thisTableCommonIndexes = new ArrayList<>();
    ...
    for (int i = 0; i < attribute.length; i++) {
        for (int j = 0; j < table2.attribute.length; j++) {
            if (attribute[i].equals(table2.attribute[j])) {
                ...
                thisTableCommonIndexes.add(i);
                table2CommonIndexes.add(j);
            }
        }
    }
    // Determine the new attributes and domains for the resulting table
    List<String> newAttributesList = new ArrayList<>(Arrays.asList(attribute));
    List<Class<?>> newDomainsList = new ArrayList<>(Arrays.<Class<?>>asList(domain));

    for (int j = 0; j < table2.attribute.length; j++) {
        if (!commonAttributes.contains(table2.attribute[j])) {
            newAttributesList.add(table2.attribute[j]);
            newDomainsList.add(table2.domain[j]);
        }
    }
    ...
    for (Comparable[] thisTuple : tuples) {
        for (Comparable[] table2Tuple : table2.tuples) {
            ...
            for (int k = 0; k < commonAttributes.size(); k++) {
                if (!thisTuple[thisTableCommonIndexes.get(k)].equals(table2Tuple[table2CommonIndexes.get(k)])) {
                    match = false;
                    break;
                }
            }
            if (match) {
                Comparable[] combinedTuple = new Comparable[newAttributes.length];
                int index = 0;
                // Add all values from this table
                for (Comparable value : thisTuple) {
                    combinedTuple[index++] = value;
                }
                
                // Add only the non-duplicate values from table2
                for (int j = 0; j < table2.attribute.length; j++) {
                    if (!table2CommonIndexes.contains(j)) {
                        combinedTuple[index++] = table2Tuple[j];
                    }
                }
    ...
    {
        try {
            new File(DIR).mkdirs(); // Ensure the directory exists
            ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(DIR + name + EXT));
            oos.writeObject(this);
            oos.close();
            out.println("Table " + name + " saved successfully.");
        } catch (IOException ex) {
            out.println("Error saving table: " + ex.getMessage());
            ex.printStackTrace();
        } // try
    } // save
    ...
    private boolean typeCheck (Comparable [] t)
    {
        ...
        for (int i = 0; i < t.length; i++) {
            if (!domain[i].isInstance(t[i])) {
                //return false if any of the element does not match the expected type.
                return false;
            }
        }
        return true;
