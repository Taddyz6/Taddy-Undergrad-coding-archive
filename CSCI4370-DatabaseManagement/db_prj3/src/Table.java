...
//        index     = new LinHashMap <> (KeyType.class, Comparable [].class);
    ...
    /**
     * Construct a table from the meta-data specifications and index selection technique.
     *
     * @param _name       the name of the relation
     * @param _attribute  the string containing attributes names
     * @param _domain     the string containing attribute domains (data types)
     * @param _key        the primary key
     * @param indexType   the specified index selection (TreeMap, HashMap, LinHashMap), if any
     */  
    public Table(String _name, String[] _attribute, Class[] _domain, String[] _key, String indexType) {
        ...
        // Choose index type based on indexType parameter
        switch (indexType) {
            case "TreeMap":
                index = new TreeMap<>();
                break;
    ...
    public Table union (Table table2)
    {
        ...
        for (Comparable[] tuple : table2.tuples) {
            if (!rows.contains(tuple)) {  // Ensure no duplicates
                rows.add(tuple);
            }
        }
    ...
    public Table minus (Table table2)
    {
        ...
        for (Comparable[] tuple : tuples) {
            if (!table2.tuples.contains(tuple)) {  // Only add if not in table2
                rows.add(tuple);
            }
        }
    ...
    public Table join (String attributes1, String attributes2, Table table2)
    {
        ...
        for (Comparable[] tuple1 : tuples) {
            Comparable[] joinKey = extract(tuple1, t_attrs);
            KeyType joinKeyVal = new KeyType(joinKey);
            
            Comparable[] tuple2 = table2.index.get(joinKeyVal);
        ...
        String[] attribute2_new = Arrays.copyOf(table2.attribute, table2.attribute.length);
        for (int j = 0; j < t_attrs.length; j++) {
            for (int k = 0; k < attribute2_new.length; ++k) {
                if (attribute2_new[k].equals(t_attrs[j])) {
                    attribute2_new[k] = attribute2_new[k] + "2";
                }
            }
        }
